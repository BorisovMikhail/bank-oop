...
def test_account_deposit_increases_balance() -> None:
    ...
    new_balance = account.deposit(5_000)
...
def test_account_withdraw_decreases_balance() -> None:
    ...
    new_balance = account.withdraw(4_000)
...
def test_bank_open_and_get_account() -> None:
    ...
    created = bank.open_account(account_id=10, owner="Alice", initial_balance=2_500)
...
def test_bank_transfer_moves_money_and_stores_transaction() -> None:
    bank = Bank()
    bank.open_account(account_id=1, owner="Alice", initial_balance=10_000)
    bank.open_account(account_id=2, owner="Bob", initial_balance=2_000)

    transaction = bank.transfer(from_id=1, to_id=2, amount=3_000)
...
def test_bank_transfer_insufficient_funds_raises() -> None:
    bank = Bank()
    bank.open_account(account_id=1, owner="Alice", initial_balance=1_000)
    bank.open_account(account_id=2, owner="Bob", initial_balance=2_000)

    with pytest.raises(ValueError, match="Insufficient funds"):
        bank.transfer(from_id=1, to_id=2, amount=3_000)
...
def test_bank_transfer_unknown_sender_raises() -> None:
    ...
    with pytest.raises(KeyError, match="not found"):
        bank.transfer(from_id=1, to_id=2, amount=500)
...
def test_bank_transfer_to_same_account_raises() -> None:
    ...
    with pytest.raises(ValueError, match="same account"):
        bank.transfer(from_id=1, to_id=1, amount=1_000)
